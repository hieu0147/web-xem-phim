...
const getBadgeText = (movie) => {
    ...
    const ep = movie.current_episode?.replace(/Tập\s*/i, '').trim() || '';
    let part = 'TM';
    if (lang.includes('Vietsub') && lang.includes('Thuyết Minh')) part = 'PĐ + TM';
    else if (lang.includes('Vietsub')) part = 'PĐ';
    return ep ? `${part}. ${ep}` : part;
};

const getBadgeClass = (movie) => {
    const lang = movie.language || '';
    if (lang.includes('Thuyết Minh') || !lang.includes('Vietsub')) return 'pd';
    return 'tm';
};
                            ...
                            <Link to={`/phim/${movie.slug}`} className="top-10-link">
                                ...
                                <div className="top-10-info">
                                    ...
                                    <div className="top-10-text">
                                        ...
                                        <p className="top-10-original">{movie.original_name}</p>
                                        <p className="top-10-episode">
                                            {movie.current_episode || (movie.created ? `(${movie.created.substring(0, 4)})` : '')}
                                        </p>
                                    </div>
                                </div>
                            </Link>
