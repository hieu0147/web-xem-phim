...
import { fetchMoviesByCountry, COUNTRY_SLUGS } from '../services/api';
...
const CountryPage = () => {
    ...
    const title = COUNTRY_SLUGS[countrySlug] ? `Phim ${COUNTRY_SLUGS[countrySlug]}` : (countrySlug || 'Danh sách phim');
    ...
    useEffect(() => {
        ...
        setPageInput('1');
        loadPage(countrySlug, 1);
    }, [countrySlug]);

    const loadPage = async (slug, page = 1) => {
        setLoading(true);
        try {
            const result = await fetchMoviesByCountry(slug, page);
                    ...
                    <>
                        {movies.length > 0 ? (
                            ...
                            <div className="no-results">
                                ...
                                <p>Quốc gia này đang cập nhật</p>
                            </div>
                        ) : null}
                    </>
                )}
